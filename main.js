...
const works = [
    ...
    },
    {
        ...
        workTerm: `2週間`,
        workPoint: `ボタンを押すたびに様々なワードが組み合わさり、おもしろい文章や恐ろしい文章が生まれます。告白を断る文章には歌の歌詞も一部混ぜてみたので、どこかで聞いたことのあるセリフもたまに出てきます。localStorageで保存、削除ができるようになっています。また、保存したデータを後から編集できる機能を実装しました。`,
    ...
    },
    {
        ...
        workSkill: `HTML/CSS/JavaScript`,
        workTerm: `製作中`,
        workPoint: `("はじめる" → "⚪︎⚪︎ちゃん編" をクリックすると見れます）ユーザーの選択によって、エンディングが変わるようになっています。バッドエンドに遷移する選択肢のある分岐点では”運命の分かれ道です。どちらを選びますか？”とポップアップが出るようにしました。シナリオはシーンデータとして配列で管理し、各シーンにそれぞれIDを付与しています。選択肢には遷移先のIDを持たせ、ユーザーの選択に応じて対応するシーンを表示するようにしました。画像はAI生成しており、選択肢によって異なる展開の画像が表示されます。`,
    ...
    {
        isBanner: true,
        workImage:`images/kamiEC.png`,
        workTitle: `ECサイトのセールのバナー(640 × 500px)`,
        workItem: `セールを心待ちにしているユーザー、金銭事情により購入を控えているユーザー`,
        workSkill: `photoshop,Illustrator`,
        workPoint: `photoshopでフリー素材の女性を漫画風に加工して、illustratorで女性を囲っている中央線と黄色のテープを作りました。友人からセールの情報を伝えられ驚いているという場面です。普段は少し高価で手が届きにくい美容家電やヘアケア商品を扱うECサイトにおいて、「今日だけ驚きの価格で手に入る」という強烈なお得感と緊急性を、ユーザーに直感的に伝えることをコンセプトに設計しました。何屋さんかわかるように周りに商品を置いています。`,
        ...
        workPoint: `夏らしさと爽やかさを演出するため、青空の背景にフレアを組み合わせ、アクセントに黄色を入れました。最も目立たせたい「果汁100%」を1番に、次に「クリームソーダ」へ視線が流れるよう動線を設計しています。また、文字の視認性を高めるため、ドロップシャドウで立体感を出して下部にカラーの背景帯を敷きました。`,
        ...
        workSkill: `Canva,photoshop`,
        workPoint: `背景は板チョコレートをパキッと割ったときの様子を表現しています。右側にシズル感のあるチョコレートソースを配置し、左と右で「静」と「動」の対比で目を引く構成にしています。主張したいのは左側のトリュフなので、左を範囲広めにしています。`,
        ...
        workSkill: `Canva,photoshop`,
        workPoint: `photoshopで手を明るく血色良くし、また左側がごちゃつかないように文字それぞれが独立して見えるようにフォントやエフェクトを工夫しました。また、"手ぶらOK"や、"今すぐ応募できます"という、気軽さをアピールしてユーザーが応募しやすくなるよう考えました。`,
        ...
        workSkill: `photoshop`,
        workPoint: `冬前のセールバナーで、氷河期が来て恐竜が絶滅してしまう前に福井に来るように促すバナーにしました。真ん中は氷河期が来て凍えている恐竜の画像を入れています。斜めの要素を入れることでスピード感を出し、ユーザーを急かしています。`,

    },
                        {
        isBanner: true,
        workImage:`images/engineerbanner.png`,
        workTitle: `SES会社の求人バナー(640 × 500px)`,
        workItem: `働き先を探している、柔軟は働き方をしたいと考えている、キャリアアップをしたいエンジニア職希望の人`,
        workSkill: `photoshop`,
        workPoint: `色合いを背景画像に合わせ、目立たせたい部分を黄色にしたり、全体的にわかりやすさを重視しました。左側は水色のグラデーションを２枚重ねて読みやすくして、紺色と青色も入れて爽やかさと信頼感をユーザーに与えられるように意識しました。`,
